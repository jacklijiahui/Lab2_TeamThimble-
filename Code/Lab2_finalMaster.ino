...
int pos=180;                // the position of hammer //0
int posLinear=100;          // position of track 
...
void pressTheKey()
{
  if(distance >=14 && distance <=16 && keyNum==3) // Key 3 is pressed down //9 11
  {
  ...
  else if(distance >= 17 && distance <= 19 && keyNum==2) // Key 2 is pressed down //15 18
  {
   ...
   if(flag2 >5){
    ...
    else if(distance >= 21 && distance <= 23 && keyNum==1) // Key 1 is pressed down //19 21
  {
...
void LinearMotion(){    //keep searching until the specified distance is detected 
    ...
    for(posLinear = 100; posLinear <= 180; posLinear += 2) // goes from 0 degrees to 180 degrees //0
      {                                  // in steps of 1 degree 
        measureTheDistance();
        servoLinear.write(posLinear);              // tell servo to go to position in variable 'pos' 
        delay(50);                       // waits 15ms for the servo to reach the position 
    
        if(distance >= 14 && distance <=16 && keyNum==3) //Key3 is find 
          { 
            if(flag3>5){
              break;
            } 
         ...
         else if(distance >= 17 && distance <=19 && keyNum==2) //Key2 is find 
          { 
            if(flag2>5){
              break;
            } 
         ...
         else if(distance >=21 && distance <=23 && keyNum==1) //Key1 is find 
          { 
            if(flag1>5){
              break;
            } 
